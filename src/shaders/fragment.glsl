...
vec4 sdScene(vec3 p) {
  ...
  vec3 color4 = vec3(vUv.y, vUv.x, 1. * uValueC);
 ...
 p.xy *= rot(vTime * .5 * -1.);
  ...
  vec4 sphereLeft = sdSphere(p3 + displacement  ,1. , vec3(0., 0, 1.0), color1 );

  vec4 cube = sdBox(vec3(p.x - 2.5, p.y- 2.5, p.z)   , vec3(1., 1., 1.0), color3 );

  vec4 torus = sdTorus(vec3(p.x + 2.5, p.y+2.5, p.z) , vec2(1.7 , .5 ), color4);

  // vec4 floorS = sdFloor(vec3(p3.x , p3.y-2.5, p3.z) , color4);
  ...
  // float blah = mix(link, link2, sin(vTime * .5) * .5 +1.);
   ...
   vec4 mixed = minWithColor(sphereLeft, cube);
   mixed = minWithColor(mixed, torus);

    // mixed = minWithColor(mixed, floorS);
  return  mixed;
}
...
{
  vec2 uv = vUv -.5;
  vec2 uv2 = uv  ;    // Scale up the space by 3
   // uv2 = fract(uv2 );
  vec3 backgroundColor = vec3(uv2.x * uValueA, uv2.y * uValueC, cnoise(vec3(uv2 *rot(vTime * .5), 1.) * 10.));



  backgroundColor = vec3(uv.x - uValueA, uv.y - uValueB,1. *uValueC);

  coswarp(backgroundColor, 3.);

  // backgroundColor += cnoise(uv2 , uValueA);



  int recursionCount = uValueD; // Number of iterations for the fractal
 float d = kochsCurve (uv2, recursionCount); // Distance from the fractal

 float lineSmoothness = .20; // Smoothness / thickness of the line (higher number = smoother / thicker, lower numbers = sharper / thinner)
   backgroundColor.r += smoothstep (lineSmoothness, 0.0, d) * 0.5;

   // Drawing the axis
   lineSmoothness = .1 ; // Descreases line smoothness for sharper / thinner lines
   backgroundColor.r = smoothstep (lineSmoothness, 0.0, length (uv2.y)); // x axis
