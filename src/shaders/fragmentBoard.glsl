  ...
  // uvRipple(uv, .3);
   ...
   vec3 color = vec3(uv.x, uv.y, 1.);
    ...
    vec2 uv3 = vec2(uv.x, uv2.y);
     ...
     float circle5 = step(distance(uv, vec2(.5)),cnoise(uv * 4.) );
   ...
   color = mix(color, color3,circle2);

   // color = mix(color, color4,circle3);
   //
   // color = mix(color, color5,circle4);
   //
   //  color = mix(color, color6,circle5);
   //  coswarp(color, 3.);
    float tri1 = stroke(triangleDF(vUv), .8,.6);
      float tri2 = stroke(triangleDF(rote), .8,.6);
      float tri3 = stroke(triangleDF(roteC), .8,.6);
    color*= tri1 + tri2 + tri3;

    // if(uv.y < uv.x){
    //   color = vec3( cnoise(uv * 3.), cnoise(uv2 * 3.), cnoise(uv3 * 3.) );
    // }
    //
